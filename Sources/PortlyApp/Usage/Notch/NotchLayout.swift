...
/// against `Codenotch's design frame` directly.
enum NotchLayout {
    // The notch body
    ...
    /// longest of them — "Portly was refused access to …'s saved login.
    ...
    /// The busiest provider that occurs — Claude, with the session, the
    /// all-models week and up to three per-model weekly windows (Fable, Opus,
    /// Sonnet). The tallest card is sized for it, since the panel is sized once
    /// for the whole stack and has to hold whichever card is worst.
    static let maxWindowCount = 5
