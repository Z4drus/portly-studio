...
final class NotchWindowController {
    ...
    /// Hooked up by the usage center; drives the menu's "Refresh now".
    var onRefresh: (() -> Void)?
    /// Refetch a single provider, asked for by clicking its ring.
    var onRefreshProvider: ((String) -> Void)?
    /// Open Portly's settings, asked for by clicking the handle.
    var onOpenSettings: (() -> Void)?
    /// Bring the main window forward, from the menu.
    var onOpenPortly: (() -> Void)?
    /// Hide the notch until Settings brings it back, from the menu.
    var onHide: (() -> Void)?
    ...
    func relocate(cellCount: Int? = nil) {
        ...
        // edge depends on.
        if let panel {
            UsageLog.usage.debug("panel \(NSStringFromRect(panel.frame), privacy: .public) on screen \(NSStringFromRect(screen.frame), privacy: .public)")
        }
    ...
    private func contextMenu() -> NSMenu {
        UsageLog.usage.debug("context menu opened")
        let menu = NSMenu()
        ...
        keepOpen.toolTip = model.isAlwaysOn
            ? "The notch is set to Always show. Change it in Portly's settings."
            : nil
        ...
        let open = NSMenuItem(
            title: "Open Portly",
            action: #selector(MenuActions.openPortly(_:)),
            keyEquivalent: ""
        )
        ...
        let hide = NSMenuItem(
            title: "Hide Notch",
            action: #selector(MenuActions.hide(_:)),
            keyEquivalent: ""
        )
        hide.target = menuActions
        hide.isEnabled = true
        hide.toolTip = "Bring it back from Portly's settings, under AI Usage."
        menu.addItem(hide)
        return menu
    }
    ...
    private lazy var menuActions = MenuActions(
        ...
        openPortly: { [weak self] in self?.onOpenPortly?() },
...
final class MenuActions: NSObject {
    ...
    init(
        ...
        openPortly: @escaping () -> Void,
        openSettings: @escaping () -> Void,
        hide: @escaping () -> Void
    ) {
        self.refresh = refresh
        self.pin = togglePinned
        self.open = openPortly
    ...
    @objc func openPortly(_ sender: Any?) { open() }
    @objc func openSettings(_ sender: Any?) { settings() }
    @objc func hide(_ sender: Any?) { hideNotch() }
}
