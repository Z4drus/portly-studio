...
/// offline, token expired — shows nothing at all. A remembered reading is
...
struct UsageArchive {
    ...
    private let key = "usage.lastGoodReadings"
    private let backoffKey = "usage.backoffUntil"
    ...
    /// When the endpoint may next be called, remembered across launches so a
    /// relaunch during a rate-limit penalty waits instead of extending it.
    /// Kept per provider: the limit is per account.
    ...
    /// Drop what we remember about one provider, so switching it off does not
    /// leave its numbers on screen at the next launch.
